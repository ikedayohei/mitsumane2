$(document).ready(function(){
  ...
  function setLabel(count) {
    ...
    if (count == 5) { 
      ...
      //id・forの値を変更
      $('.label-box').attr({for: `estimate_images_attributes_${count}_image`});
    }
  }

  //編集ページ(items/:i/edit)へリンクした際のアクション==================================
  if (window.location.href.match(/\/estimates\/\d+\/edit/)){
    //プレビューの数を取得
  ...
  $(document).on('change', '.hidden-field', function() {
    ...
    reader.onload = function() {
      ...
      if ($(`#estimate_images_attributes_${id}__destroy`)){
        $(`#estimate_images_attributes_${id}__destroy`).prop('checked',false);
      }
      ////プレビューの数を取得
      var count = $('.preview-box').length;
      //countに応じてラベルのwidth・id・forの値を変更
  ...
  $(document).on('click', '.delete-box', function() {
    ...
    var id = $(this).attr('id').replace(/[^0-9]/g, '');
    if ($(`#estimate_images_attributes_${id}__destroy`).length) {
      $(`#estimate_images_attributes_${id}__destroy`).prop('checked',true);
    }
    //取得したidに該当するプレビューを削除
    $(`#preview-box__${id}`).remove();
    //フォームの中身を削除 
    $(`#estimate_images_attributes_${id}_image`).val("");
    //プレビューの数を取得
    //countに応じてラベルのwidth・id・forの値を変更
    setLabel(id);
  });
